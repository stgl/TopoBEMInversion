...
G_chan = load('input/G_tect_shear_push_simple.txt');
G_bay = load('input/G_bay_shear_push_simple.txt');
G_lp = load('input/G_lomaprieta_shear_push_simple.txt');
...
try
    ...
    figure
    scatter(x_channels,y_channels,2,e_chan)
    colorbar
    % caxis([0 1000])
    axis equal
    title('Data')
    ...
    figure
    scatter(x_channels,y_channels,2,elev_pred_outletsfree)
    colorbar
    % caxis([0 1000])
    axis equal
    title('Predicted Elevation outlets')

    figure
    scatter(x_channels,y_channels,2,residuals_elev_outletsfree)
    colorbar, colormap(flipud(jet))
    % caxis([-300 300])
    axis equal
    title('Residuals outlets')
    ...
    save singleK_strong_enforcement
